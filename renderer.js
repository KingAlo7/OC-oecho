 ...
 *
 * Requires the patched smiles-drawer bundle loaded BEFORE this file (must
 * expose window.SmilesDrawer). The patch makes the arrow auto-grow to fit
 * single-line reagent text — i.e. the library will NEVER wrap reagent
 * labels to multiple rows above the arrow.
 *
 * Naming note: the patched smiles-drawer bundle is currently stored in
 * the repo root as `reaction-drawer.js` (this is a historical accident —
 * a file rename in the repo would have been cleaner, but the bundle bytes
 * were uploaded under that name). This file (`renderer.js`) holds the
 * thin wrapper that USES window.SmilesDrawer.
 *
 * Usage:
...
const ReactionRenderer = (() => {
  ...
  /* Molecule rendering options.
     ...
     reaction draws atoms at the same physical scale — this is what
